...
export default function Press({data}) {
  ...
  return (
    ...
    {data.map(item=>(
      <motion.div initial={{x:'-100vw'}} animate={{x:0,transition:{duration:item.id}}} whileHover={{ scale: 1.03 }} transition={{ type: "spring", stiffness: 500, damping: 10 }} key={item.id} className='bg-primary-light-green drop-shadow-lg p-2 rounded-md'>
      <Link href={`/press/type/${item.type}`} legacyBehavior><a className='w-full'>
          <div className='grid grid-cols-2  gap-4'>
            ...
            <div className='flex  justify-center items-center'>
              <h1 className='font-bold text-xl'>{item.section}</h1>
            </div>
          </div>
          </a></Link>
    </motion.div>
...
export async function getStaticProps() {
  const {data}=await axios.get(`http://localhost:4000/pressKind`)
