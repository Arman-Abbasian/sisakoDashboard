    ...
    <div className='p-2'>
     <Head>
      ...
      <main className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10 py-4  mt-10 mb-20 container mx-auto'>
        {data.map(item=>{
          return <motion.div initial={{opacity:0,x:'100vw'}} animate={{opacity:1,rotate: 360,x:0,transition:{duration:2}}}  whileHover={{scale:1.05,transition:{duration:0.5,stiffness:400,type: "spring", bounce: 4,damping:6,mass: 3}}} key={item.id} className='bg-primary-light-green drop-shadow-lg p-2 rounded-md'>
