...
export default function PersonnelUnit({data,typeSlug}) {
  ...
  return (
    ...
    {data && 
    data.map(item=>{
        return  <motion.div variants={sectionAnimation} initial="hidden" animate="visible" whileHover="hover" key={item.id} className='bg-primary-light-green drop-shadow-lg p-2 rounded-md'>
      <Link key={item.id} href={`/personnel/type/${typeSlug}/${item.unit}`} legacyBehavior><a className='w-full'>
          <div className='grid grid-cols-2  gap-4'>
            ...
            <div className='flex  justify-center items-center'>
              <h1 className='font-bold text-xl'>{item.unit}</h1>
            </div>
          </div>
          </a></Link>
    </motion.div>
