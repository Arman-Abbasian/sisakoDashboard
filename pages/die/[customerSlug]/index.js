...
export default function Customer({customer}) {
  console.log(customer)
   ...
   useEffect(()=>{
    ...
    axios.get(`http://localhost:4000/products?customer=${customer}`)
    .then((res)=>{setProducts({data:res.data,error:"",loading:false});
    })
    .catch(err=>{
        setProducts({data:null,error:err.message,loading:false})
  ...
  return (
    ...
    {products.data && 
      ...
      <Link href={`/die/${item.customer}/${item.product}`} legacyBehavior><a className='w-full'>
          <div className='grid grid-cols-2  gap-4'>
            ...
            <div className='flex  justify-center items-center'>
              <h1 className='font-bold text-xl'>{item.product}</h1>
            </div>
          </div>
          </a></Link>
    </motion.div>
...
export async function getStaticPaths() {
 ...
 const paths=data.map(item=>{
  return { params: { customerSlug:item.customer } }
 });
 console.log(paths)
    return {
      paths:paths,
