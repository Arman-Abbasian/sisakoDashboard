...
export default function Part({data}) {
  console.log(data)
   ...
   useEffect(()=>{
    ...
    axios.get(`http://localhost:4000/parts?product=${data.productSlug}&customer=${data.customerSlug}`)
    .then((res)=>{
        setParts({data:res.data,errorr:"",loading:false});
    })
    .catch(err=>{
  ...
  return (
    ...
    {parts.data && 
        parts.data.map((item)=>(
        <motion.div variants={sectionAnimation} initial="hidden" animate="visible" whileHover="hover" className='bg-primary-light-green drop-shadow-lg p-2 rounded-md'>
          <Link key={item.id} href={`/die/${data.customerSlug}/${data.productSlug}/${item.part}`} legacyBehavior><a className='w-full'>
            <div className='grid grid-cols-2  gap-4'>
              ...
              <div className='flex  justify-center items-center'>
                <h1 className='font-bold text-xl'>{item.part}</h1>
              </div>
            </div>
          </a></Link>
    </motion.div>
